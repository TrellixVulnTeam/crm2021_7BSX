...
class UsuarioController extends Controller
{
        ...
        public function validarCredenciales(Request $request)
        {
			...
			{     
                ...
                select top 1 u.* ,fe.* from users u 
                inner join usuario_rol ur on ur.user_id =u.id 
                INNER JOIN roles r on r.id = ur.rol_id 
				left join FACTURACION.dbo.fe_usuarios fe on fe.id_comanda = u.id
        ...
        public function getUsuarios(){
            ...
            $users = DB::connection('comanda')->select("SELECT *, concat(nombre, ' ', apellido) as nombre_apellido from users where (estado is null or estado = 1) order by 2 asc");
